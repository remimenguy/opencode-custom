...
function patchClaudeCodeIntegration(text) {
  ...
  if (!text.includes("var CODEX_PROVIDER_ID")) {
    text = replaceOnce(
      text,
      "function custom6(dep) {",
      "var CODEX_PROVIDER_ID = \"codex\", CODEX_PACKAGE = \"@opencode/codex\";\nvar CODEX_EFFORTS = [\"low\", \"medium\", \"high\", \"xhigh\"];\nfunction isCodexProviderID(providerID2) {\n  return String(providerID2) === CODEX_PROVIDER_ID;\n}\nfunction codexNormalizeEffort(value4) {\n  if (typeof value4 !== \"string\")\n    return;\n  const normalized = value4.trim().toLowerCase();\n  return CODEX_EFFORTS.includes(normalized) ? normalized : void 0;\n}\nfunction codexEffortFromProviderOptions(providerOptions) {\n  return codexNormalizeEffort(providerOptions?.[CODEX_PROVIDER_ID]?.effort);\n}\nfunction codexRequestEffort(request32) {\n  return codexEffortFromProviderOptions(request32?.providerOptions) ?? codexEffortFromProviderOptions(request32?.model?.route?.defaults?.providerOptions);\n}\nfunction codexModelsDevProvider() {\n  const model = (id12, name38, family, costInput, costOutput) => ({\n    id: id12,\n    name: name38,\n    family,\n    status: \"active\",\n    release_date: \"2026-06-15\",\n    cost: { input: costInput, output: costOutput, cache_read: 0, cache_write: 0 },\n    limit: { context: 4e5, output: 128e3 },\n    modalities: { input: [\"text\"], output: [\"text\"] },\n    temperature: false,\n    reasoning: true,\n    reasoning_options: [{ type: \"effort\", values: CODEX_EFFORTS }],\n    attachment: false,\n    tool_call: false\n  });\n  return {\n    id: CODEX_PROVIDER_ID,\n    name: \"Codex\",\n    env: [],\n    npm: CODEX_PACKAGE,\n    api: \"\",\n    models: {\n      \"gpt-5.5\": model(\"gpt-5.5\", \"Codex GPT-5.5\", \"codex-gpt-5-5\", 1.25, 10)\n    }\n  };\n}\nfunction codexProviderInfo() {\n  return fromModelsDevProvider(codexModelsDevProvider());\n}\nfunction codexSpawnEnv() {\n  const env4 = { ...process.env };\n  const pathParts = [\n    path78__default.join(OS__default.homedir(), \".local/bin\"),\n    \"/opt/homebrew/bin\",\n    \"/usr/local/bin\",\n    env4.PATH\n  ].filter(Boolean);\n  env4.PATH = [...new Set(pathParts.join(path78__default.delimiter).split(path78__default.delimiter).filter(Boolean))].join(path78__default.delimiter);\n  delete env4.ELECTRON_RUN_AS_NODE;\n  return env4;\n}\nfunction findCodexBinary() {\n  const envPath = process.env.CODEX_PATH;\n  const candidates = [\n    envPath,\n    \"/opt/homebrew/bin/codex\",\n    \"/usr/local/bin/codex\",\n    path78__default.join(OS__default.homedir(), \".local/bin/codex\"),\n    ...String(process.env.PATH ?? \"\").split(path78__default.delimiter).filter(Boolean).map((item2) => path78__default.join(item2, \"codex\"))\n  ].filter((item2) => typeof item2 === \"string\" && item2.trim() !== \"\");\n  for (const item2 of candidates) {\n    try {\n      if (fs10__default.existsSync(item2))\n        return item2;\n    } catch {\n    }\n  }\n  try {\n    const result5 = NodeChildProcess__default.spawnSync(\"codex\", [\"--version\"], { env: codexSpawnEnv(), stdio: \"ignore\" });\n    if (result5.status === 0)\n      return \"codex\";\n  } catch {\n  }\n  return;\n}\nfunction codexUsageFromRaw(usage3) {\n  if (!usage3 || typeof usage3 !== \"object\")\n    return;\n  const input = usage3.input_tokens;\n  const output2 = usage3.output_tokens;\n  const cacheRead = usage3.cached_input_tokens;\n  const entries5 = Object.entries({\n    inputTokens: input,\n    outputTokens: output2,\n    totalTokens: typeof input === \"number\" && typeof output2 === \"number\" ? input + output2 : void 0,\n    cacheReadInputTokens: cacheRead\n  }).filter((entry) => typeof entry[1] === \"number\" && Number.isFinite(entry[1]));\n  return entries5.length === 0 ? void 0 : Object.fromEntries(entries5);\n}\nfunction codexSystemText(system) {\n  const base5 = Array.isArray(system) ? system.map(claudeCodePartText).filter((item2) => item2.trim() !== \"\").join(\"\\n\\n\") : claudeCodeStringify(system);\n  return [\n    base5,\n    \"You are running as the Codex provider inside OpenCode. OpenCode owns tool execution, file edits, permissions, and conversation state. Do not run commands or edit files yourself; answer directly from the context provided in the prompt.\"\n  ].filter((item2) => item2.trim() !== \"\").join(\"\\n\\n\");\n}\nfunction codexPromptFromAI(options10) {\n  const prompt = options10.prompt ?? [];\n  const system = prompt.filter((message32) => message32.role === \"system\").map((message32) => claudeCodeContentText(message32.content)).join(\"\\n\\n\");\n  const messages3 = prompt.filter((message32) => message32.role !== \"system\");\n  return {\n    system: codexSystemText([options10.system, system].filter(Boolean).join(\"\\n\\n\")),\n    prompt: claudeCodePromptText(messages3)\n  };\n}\nfunction codexPromptFromLLM(request32) {\n  return {\n    system: codexSystemText(request32.system ?? []),\n    prompt: claudeCodePromptText(request32.messages ?? [])\n  };\n}\nfunction codexPromptText(parts) {\n  return [parts.system, parts.prompt].filter((item2) => typeof item2 === \"string\" && item2.trim() !== \"\").join(\"\\n\\n\");\n}\nasync function* codexTextStream(input) {\n  const command = input.command ?? findCodexBinary();\n  if (!command)\n    throw new Error(\"Codex CLI not found. Install Codex or set CODEX_PATH.\");\n  const args4 = [\n    \"exec\",\n    \"--json\",\n    \"--skip-git-repo-check\",\n    \"--ephemeral\",\n    \"--color\",\n    \"never\",\n    \"-s\",\n    \"read-only\",\n    \"-m\",\n    input.model\n  ];\n  const effort4 = codexNormalizeEffort(input.effort);\n  if (effort4)\n    args4.push(\"-c\", `model_reasoning_effort=${effort4}`);\n  args4.push(\"-\");\n  const child = NodeChildProcess__default.spawn(command, args4, {\n    env: codexSpawnEnv(),\n    stdio: [\"pipe\", \"pipe\", \"pipe\"]\n  });\n  const queue = [];\n  let stdout = \"\";\n  let stderr = \"\";\n  let done3 = false;\n  let error46;\n  let notify;\n  const wake = () => {\n    if (notify) {\n      const fn3 = notify;\n      notify = void 0;\n      fn3();\n    }\n  };\n  const push = (item2) => {\n    queue.push(item2);\n    wake();\n  };\n  const handleLine = (line6) => {\n    const trimmed = line6.trim();\n    if (!trimmed || trimmed[0] !== \"{\")\n      return;\n    let event2;\n    try {\n      event2 = JSON.parse(trimmed);\n    } catch {\n      return;\n    }\n    const type = event2.type;\n    if (type === \"item.completed\" && event2.item && event2.item.type === \"agent_message\" && typeof event2.item.text === \"string\") {\n      if (event2.item.text.length > 0)\n        push({ type: \"text\", text: event2.item.text });\n    } else if (type === \"turn.completed\") {\n      const usage3 = codexUsageFromRaw(event2.usage);\n      if (usage3)\n        push({ type: \"usage\", usage: usage3 });\n    } else if (type === \"error\") {\n      let message = event2.message;\n      try {\n        const parsed = JSON.parse(message);\n        message = parsed?.error?.message ?? message;\n      } catch {\n      }\n      error46 = new Error(typeof message === \"string\" ? message : \"Codex failed\");\n      done3 = true;\n      wake();\n    }\n  };\n  child.stdout.on(\"data\", (chunk) => {\n    stdout += chunk.toString(\"utf8\");\n    let index3;\n    while ((index3 = stdout.indexOf(\"\\n\")) !== -1) {\n      const line6 = stdout.slice(0, index3);\n      stdout = stdout.slice(index3 + 1);\n      handleLine(line6);\n    }\n  });\n  child.stderr.on(\"data\", (chunk) => {\n    stderr += chunk.toString(\"utf8\");\n  });\n  child.on(\"error\", (e3) => {\n    error46 = e3;\n    done3 = true;\n    wake();\n  });\n  child.on(\"close\", (code, signal) => {\n    if (stdout.trim()) {\n      handleLine(stdout);\n      stdout = \"\";\n    }\n    if (!error46 && code !== 0) {\n      const suffix = signal ? ` signal ${signal}` : ` code ${code}`;\n      error46 = new Error((stderr.trim() || `Codex exited with${suffix}`).trim());\n    }\n    done3 = true;\n    wake();\n  });\n  if (input.abortSignal) {\n    input.abortSignal.addEventListener(\"abort\", () => {\n      try {\n        child.kill(\"SIGTERM\");\n      } catch {\n      }\n    }, { once: true });\n  }\n  child.stdin.end(input.prompt);\n  try {\n    while (!done3 || queue.length > 0) {\n      if (queue.length > 0) {\n        yield queue.shift();\n        continue;\n      }\n      await new Promise((resolve4) => notify = resolve4);\n    }\n    if (error46)\n      throw error46;\n  } finally {\n    try {\n      child.kill(\"SIGTERM\");\n    } catch {\n    }\n  }\n}\nfunction createCodexLanguageModel(modelID, options10 = {}) {\n  return {\n    specificationVersion: \"v3\",\n    provider: CODEX_PROVIDER_ID,\n    modelId: modelID,\n    supportedUrls: {},\n    async doGenerate(options11) {\n      const { system, prompt } = codexPromptFromAI(options11);\n      let text82 = \"\";\n      let usage3;\n      for await (const item2 of codexTextStream({ command: options10.command, model: modelID, prompt: codexPromptText({ system, prompt }), effort: codexEffortFromProviderOptions(options11.providerOptions), abortSignal: options11.abortSignal })) {\n        if (item2.type === \"usage\")\n          usage3 = item2.usage;\n        if (item2.type === \"text\")\n          text82 += item2.text;\n      }\n      return {\n        content: [{ type: \"text\", text: text82 }],\n        finishReason: { unified: \"stop\", raw: \"stop\" },\n        usage: claudeCodeAIUsage(usage3),\n        warnings: [],\n        response: { headers: {} },\n        request: { body: { model: modelID } }\n      };\n    },\n    async doStream(options11) {\n      const { system, prompt } = codexPromptFromAI(options11);\n      const command = options10.command;\n      const stream11 = new ReadableStream({\n        async start(controller) {\n          let textStarted = false;\n          let usage3;\n          controller.enqueue({ type: \"stream-start\", warnings: [] });\n          try {\n            for await (const item2 of codexTextStream({ command, model: modelID, prompt: codexPromptText({ system, prompt }), effort: codexEffortFromProviderOptions(options11.providerOptions), abortSignal: options11.abortSignal })) {\n              if (item2.type === \"usage\") {\n                usage3 = item2.usage;\n                continue;\n              }\n              if (!textStarted) {\n                controller.enqueue({ type: \"text-start\", id: \"text-0\" });\n                textStarted = true;\n              }\n              controller.enqueue({ type: \"text-delta\", id: \"text-0\", delta: item2.text });\n            }\n            if (textStarted)\n              controller.enqueue({ type: \"text-end\", id: \"text-0\" });\n            controller.enqueue({ type: \"finish\", finishReason: { unified: \"stop\", raw: \"stop\" }, usage: claudeCodeAIUsage(usage3) });\n            controller.close();\n          } catch (e3) {\n            controller.enqueue({ type: \"error\", error: e3 });\n            controller.enqueue({ type: \"finish\", finishReason: { unified: \"error\", raw: \"error\" }, usage: claudeCodeAIUsage() });\n            controller.close();\n          }\n        }\n      });\n      return { stream: stream11, request: { body: { model: modelID } }, response: { headers: {} } };\n    }\n  };\n}\nfunction codexPreparedRequest(request32) {\n  return new PreparedRequest({\n    id: request32.id ?? \"request\",\n    route: CODEX_PROVIDER_ID,\n    protocol: CODEX_PROVIDER_ID,\n    model: request32.model,\n    body: { model: request32.model.id },\n    metadata: { transport: \"codex-cli\" }\n  });\n}\nfunction codexLLMClientStream(request32) {\n  return fromAsyncIterable2((async function* () {\n    const { system, prompt } = codexPromptFromLLM(request32);\n    let textStarted = false;\n    let usage3;\n    yield LLMEvent.stepStart({ index: 0 });\n    try {\n      for await (const item2 of codexTextStream({ model: request32.model.id, prompt: codexPromptText({ system, prompt }), effort: codexRequestEffort(request32), abortSignal: request32.abort })) {\n        if (item2.type === \"usage\") {\n          usage3 = item2.usage;\n          continue;\n        }\n        if (!textStarted) {\n          yield LLMEvent.textStart({ id: \"text-0\" });\n          textStarted = true;\n        }\n        yield LLMEvent.textDelta({ id: \"text-0\", text: item2.text });\n      }\n      if (textStarted)\n        yield LLMEvent.textEnd({ id: \"text-0\" });\n      yield LLMEvent.stepFinish({ index: 0, reason: \"stop\", usage: usage3 });\n      yield LLMEvent.finish({ reason: \"stop\", usage: usage3 });\n    } catch (e3) {\n      if (textStarted)\n        yield LLMEvent.textEnd({ id: \"text-0\" });\n      yield LLMEvent.providerError({ message: claudeCodeErrorMessage(e3), retryable: true });\n      yield LLMEvent.stepFinish({ index: 0, reason: \"error\" });\n      yield LLMEvent.finish({ reason: \"error\" });\n    }\n  })(), (e3) => e3 instanceof Error ? e3 : new Error(String(e3)));\n}\nfunction custom6(dep) {",
      "Codex helper runtime"
    );
  }
  if (!text.includes("\"codex\": () => exports_Effect.succeed")) {
    text = replaceOnce(
      text,
      "function custom6(dep) {\n  return {\n",
      "function custom6(dep) {\n  return {\n    \"codex\": () => exports_Effect.succeed({\n      autoload: findCodexBinary() !== void 0,\n      options: {\n        command: findCodexBinary() ?? \"codex\"\n      },\n      async getModel(_sdk, modelID, options10) {\n        return createCodexLanguageModel(modelID, { command: options10?.command });\n      }\n    }),\n",
      "Codex custom provider"
    );
  }
  if (!text.includes("data2[CODEX_PROVIDER_ID] = codexModelsDevProvider()")) {
    text = replaceOnce(
      text,
      "        if (findClaudeCodeBinary())\n          data2[CLAUDE_CODE_PROVIDER_ID] = claudeCodeModelsDevProvider();\n",
      "        if (findClaudeCodeBinary())\n          data2[CLAUDE_CODE_PROVIDER_ID] = claudeCodeModelsDevProvider();\n        if (findCodexBinary())\n          data2[CODEX_PROVIDER_ID] = codexModelsDevProvider();\n",
      "Codex models.dev injection"
    );
  }
  if (!text.includes("catalog[CODEX_PROVIDER_ID] = codexProviderInfo()")) {
    text = replaceOnce(
      text,
      "      if (findClaudeCodeBinary())\n        catalog[CLAUDE_CODE_PROVIDER_ID] = claudeCodeProviderInfo();\n",
      "      if (findClaudeCodeBinary())\n        catalog[CLAUDE_CODE_PROVIDER_ID] = claudeCodeProviderInfo();\n      if (findCodexBinary())\n        catalog[CODEX_PROVIDER_ID] = codexProviderInfo();\n",
      "Codex provider database injection"
    );
  }
  if (!text.includes("resolved.api.package === CODEX_PACKAGE")) {
    text = replaceOnce(
      text,
      "  if (resolved.api.type === \"aisdk\" && resolved.api.package === CLAUDE_CODE_PACKAGE) {\n    const claudeCodeRouteEffort = claudeCodeNormalizeEffort(resolved.request.body?.effort);\n    return exports_Effect.succeed(route2.with({\n      provider: CLAUDE_CODE_PROVIDER_ID,\n      endpoint: { baseURL: \"http://localhost\" },\n      providerOptions: claudeCodeRouteEffort ? { [CLAUDE_CODE_PROVIDER_ID]: { effort: claudeCodeRouteEffort } } : void 0,\n      limits: { context: resolved.limit.context, output: resolved.limit.output }\n    }).model({ id: resolved.api.id }));\n  }\n",
      "  if (resolved.api.type === \"aisdk\" && resolved.api.package === CLAUDE_CODE_PACKAGE) {\n    const claudeCodeRouteEffort = claudeCodeNormalizeEffort(resolved.request.body?.effort);\n    return exports_Effect.succeed(route2.with({\n      provider: CLAUDE_CODE_PROVIDER_ID,\n      endpoint: { baseURL: \"http://localhost\" },\n      providerOptions: claudeCodeRouteEffort ? { [CLAUDE_CODE_PROVIDER_ID]: { effort: claudeCodeRouteEffort } } : void 0,\n      limits: { context: resolved.limit.context, output: resolved.limit.output }\n    }).model({ id: resolved.api.id }));\n  }\n  if (resolved.api.type === \"aisdk\" && resolved.api.package === CODEX_PACKAGE) {\n    const codexRouteEffort = codexNormalizeEffort(resolved.request.body?.effort);\n    return exports_Effect.succeed(route2.with({\n      provider: CODEX_PROVIDER_ID,\n      endpoint: { baseURL: \"http://localhost\" },\n      providerOptions: codexRouteEffort ? { [CODEX_PROVIDER_ID]: { effort: codexRouteEffort } } : void 0,\n      limits: { context: resolved.limit.context, output: resolved.limit.output }\n    }).model({ id: resolved.api.id }));\n  }\n",
      "Codex route support"
    );
  }
  if (!text.includes("model8.api.package === CODEX_PACKAGE")) {
    text = replaceOnce(
      text,
      "model8.api.package === CLAUDE_CODE_PACKAGE || model8.api.package === \"@ai-sdk/openai-compatible\"",
      "model8.api.package === CLAUDE_CODE_PACKAGE || model8.api.package === CODEX_PACKAGE || model8.api.package === \"@ai-sdk/openai-compatible\"",
      "Codex supported predicate"
    );
  }
  if (!text.includes("return codexPreparedRequest(request32)")) {
    text = replaceOnce(
      text,
      "if (request32.model.provider === CLAUDE_CODE_PROVIDER_ID)\n      return claudeCodePreparedRequest(request32);\n",
      "if (request32.model.provider === CLAUDE_CODE_PROVIDER_ID)\n      return claudeCodePreparedRequest(request32);\n    if (request32.model.provider === CODEX_PROVIDER_ID)\n      return codexPreparedRequest(request32);\n",
      "Codex prepared request"
    );
  }
  if (!text.includes("streamWithCodex")) {
    text = replaceOnce(
      text,
      "const streamWithClaudeCode = (request32) => request32.model.provider === CLAUDE_CODE_PROVIDER_ID ? claudeCodeLLMClientStream(request32) : stream82(request32);\n    return Service5.of({ prepare: prepareWith, stream: streamWithClaudeCode, generate: generateWith(streamWithClaudeCode) });",
      "const streamWithClaudeCode = (request32) => request32.model.provider === CLAUDE_CODE_PROVIDER_ID ? claudeCodeLLMClientStream(request32) : stream82(request32);\n    const streamWithCodex = (request32) => request32.model.provider === CODEX_PROVIDER_ID ? codexLLMClientStream(request32) : streamWithClaudeCode(request32);\n    return Service5.of({ prepare: prepareWith, stream: streamWithCodex, generate: generateWith(streamWithCodex) });",
      "Codex LLMClient stream"
    );
  }
  if (!text.includes("isCodexProviderID(model8.providerID)")) {
    text = replaceOnce(
      text,
      "      if (isClaudeCodeProviderID(model8.providerID)) {",
      "      if (isCodexProviderID(model8.providerID)) {\n        const command = provider11?.options?.command;\n        const language2 = createCodexLanguageModel(model8.api.id, { command: typeof command === \"string\" ? command : void 0 });\n        s6.models.set(key3, language2);\n        return language2;\n      }\n      if (isClaudeCodeProviderID(model8.providerID)) {",
      "Codex Provider.getLanguage"
    );
  }
  return text;
}
